...
| `lint`              | `deno run gen:types && oxlint`                                                            | passes — 0 warnings / 0 errors                |
| `fmt` / `fmt:check` | `oxfmt` (single quotes, semis, width 120)                                                 | passes; run `deno task fmt` before committing |
| `test`              | `deno run gen:types && vitest run`                                                        | passes (11 files / 102 tests)                 |
| `build` / `start`   | `vite build` (Nitro `deno-deploy` preset → `.output/server/index.mjs`), then `node` on it | build output is gitignored                    |

- Run a single test via `vitest run <path-or-pattern>` (e.g. `vitest run src/test/analysis.test.ts`); skip the `gen:types` prefix if `deno.d.ts` is already present.
...
- `oxlint` uses an **explicit rule list** (no ESLint recommended sets) + type-aware rules (`oxlint-tsgolint` backend, needs the regenerated `deno.d.ts`) + `eslint-plugin-react-you-might-not-need-an-effect` via `jsPlugins`. Rules encode the `ferrousaurus-react-best-practices` / `ferrousaurus-typescript-best-practices` skills: no `React.FC`, no class components, no `any`, no non-null assertions, no nested ternaries, `exhaustive-deps` = error, `type` over `interface`. A few pure-noise rules are recalibrated in `oxlint.config.ts` (e.g. `id-length`, `no-magic-numbers`, `no-null`, `no-array-sort`, complexity limits) — each relaxation carries a rationale comment there; meaningful correctness rules stay active.
