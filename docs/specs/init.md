...
Version note: this rewrite (2026-08-11) supersedes the original spec. The design changed materially during the review interview: the ADP source became multi-platform (BeatADP + league-aware ESPN), the regression moved from Projected Points to VORP, and caching gained a KV layer for BeatADP. Section 9 records the probe results that drove the changes.

A subsequent review (same date, 2026-08-11) migrated the runtime/deploy target from Cloudflare Workers + Cloudflare KV to Deno Deploy + Deno KV after discovering that the `@deno/kv` Node client targets only Deploy Classic (sunset 2026-07-20); Deno KV is reachable only from the Deno runtime itself. Section 10 records the deviation from `ferrousaurus-stack-preferences`.

A later change (same date, 2026-08-11) adopted **Varlock** (varlock.dev) for environment-variable validation: the committed `.env.schema` is the single source of truth for the test-only ESPN credentials, validated and injected at dev/build/test time via the Vite integration and `varlock run`; secret values live only in the gitignored `.env.local`. Section 10.2 records the schema, the wiring, and the security boundary. Varlock is the project's stack-preferred secrets-management tool (per `ferrousaurus-stack-preferences`), so no §10.1 deviation is recorded.
...
1. **RESOLVED (2026-08-11 live probes): ESPN ADP endpoint.** The league-scoped kona pattern (§5.3) is validated end-to-end: `view=kona_player_info` + `X-Fantasy-Filter` + cookies returns the full player pool (fetched with `limit: 1000`) with `ownership.averageDraftPosition` for every player in it. The former fallback (rank per variant) is retained for the rare null-ADP case. `fantasy.espn.com` redirects to a marketing page — the provider must use `lm-api-reads`. The anonymous endpoints are useless (401 / fixed 50-player stub); there is no public mode. The `proTeamId` space is classic numbering (§5.3) — the sandbox-derived map in the original spec was partially wrong and is replaced.
2. **RESOLVED: QB `FPS` vs `Custom` discrepancy** (~5.6 pts for Josh Allen) — the engine reproduces `Custom` exactly at workbook-default settings for all players; master `FPS` is a separate 0-PPR formula (appears computed from rounded inputs). The oracle targets `Custom`; documented in `tests/scoring.test.ts`.
3. **OPEN: Workbook `Player ID` space** — identified as not-ESPN, not-Sleeper; left opaque. If the user later identifies the space and it matches an ADP source, ID-based matching becomes an option (spec change would be required).
4. **OPEN (accepted risk): BeatADP HTML-scraping fragility.** BeatADP has no JSON API; markup changes could break the parser. Mitigation: parsing isolated in one module, fixture-tested, and the Deno KV cache limits fetch frequency. Re-verify the table shape on each BeatADP cache miss until stable.
5. **OPEN (known limitation): BeatADP 2QB coverage.** Only `HALF_PPR/REDRAFT/2QB` currently returns data; PPR/2QB and STANDARD/2QB are empty. Surfaced as a banner (§5.2). Re-check periodically.
6. **DEFERRED: deployment mechanism.** The Deno Deploy deploy mechanism (`deno deploy` CLI vs GitHub auto-build; KV database assignment in the Deno Deploy dashboard) is post-v1. In dev the BeatADP cache runs against Deno's in-memory `Deno.openKv()` backing (no dashboard setup needed), so the code path is fully testable now. No in-repo deploy config exists in v1 — there is no Deno-side equivalent of `wrangler.toml`; the only out-of-band configuration is the KV database assignment in the Deno Deploy dashboard.

## 10. Deployment & Stack

- **Runtime/toolchain**: **Deno CLI** — chosen greenfield because Deno KV is reachable only from the Deno runtime (the `@deno/kv` Node client targets only Deploy Classic, sunset 2026-07-20). TanStack Start in SPA mode over a Nitro `deno_deploy` preset build; **deployed to Deno Deploy** (mechanism deferred, §9.6). Dev: `deno task dev` (invokes Vite via Deno's npm-compat layer, preserving Vite HMR; the Varlock Vite plugin loads and validates the env at startup, §10.2). Test: `deno task test` (runs `varlock run -- deno x -E vitest run` — Varlock resolves and validates the `VITEST_ESPN_*` vars into the test process, §10.2 — with `Deno.openKv()` returning an in-memory backing store). No Miniflare — Deno's in-memory KV backing serves the KV role in dev and tests.
...
The skill rule files are left unchanged — the breaches are app-specific, not project-wide. The original spec also breached the Cloudflare-Unacceptable rule; this rewrite carries forward the same app-specific posture under a different vendor.
...
- **Wiring:** `@varlock/vite-integration`'s `varlockVitePlugin` in `vite.config.ts` loads and validates the resolved env at dev/build time — misconfiguration fails fast with a clear message before any app code runs (no silent `undefined`). The test command wraps vitest: `deno task test` → `varlock run -- deno x -E vitest run` — Varlock resolves and validates the vars, injects them into the child process, and `deno x` (Deno's npx equivalent) executes the vitest npm binary with `-E` (env-read permission) so the test code sees them as plain `process.env` vars. The app never imports `varlock/auto-load` or the typed `ENV` object — there is no runtime env surface to type or to leak into client bundles (the `VITEST_` names carry no `VITE_` prefix, so Vite's client-side `import.meta.env` never exposes them regardless).
...
13. `deno task dev`, `deno task lint` (chains `deno types > deno.d.ts &&` before `oxlint` with `eslint-plugin-react-you-might-not-need-an-effect` and type-aware linting enabled), `deno task fmt` (runs `oxfmt`), `deno task check` (chains `deno types > deno.d.ts &&` before `tsc --noEmit`), and `deno task test` (runs `varlock run -- deno x -E vitest run` under Deno) all pass. CI runs `deno task -A <name>` where Deno permission prompts would otherwise block non-interactively.
14. Environment variables are validated by Varlock (§10.2): `.env.schema` is committed and contains keys + decorators only (never values); `.env.local` is gitignored; dev/build validate through the Vite integration and tests through `varlock run`, with misconfiguration failing fast and with a clear message; the league-aware ESPN tests read `VITEST_ESPN_S2` / `VITEST_SWID` / `VITEST_ESPN_LEAGUE` from the environment and skip cleanly when absent; `varlock scan` runs in CI and finds no leaked secrets.
