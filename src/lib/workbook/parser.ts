...
function isSkippedName(name: string): boolean {
  const n = name.trim();
  return n === '' || n === '0' || /^\d+$/.test(n);
}

function rowsOf(wb: XLSX.WorkBook, sheetName: string): Array<Array<unknown>> {
  const ws: Sheet | undefined = wb.Sheets[sheetName];
  if (ws === undefined) return [];
  return XLSX.utils.sheet_to_json<Array<unknown>>(ws, { header: 1, raw: true, defval: undefined });
}
...
type MasterSpec = {
  ...
  teamCol: number;
  byeCol: number;
  stats: ReadonlyArray<[keyof RawStats, number]>;
  customCol: number;
  refCol: number;
};

const MASTER_SPECS: ReadonlyArray<{ sheet: string; spec: MasterSpec }> = [
  {
    sheet: 'QB',
    spec: {
      position: 'QB',
...
function parseDstStats(rows: Array<Array<unknown>>): Map<string, RawStats> {
...
const RANKINGS_BLOCKS: ReadonlyArray<{ position: Position; nameCol: number; teamCol: number; idCol: number }> = [
...
function parseRankings(rows: Array<Array<unknown>>): Map<string, { playerId: number | null; team: string | null }> {
