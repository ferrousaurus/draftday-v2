...
describe('boardToCsv content', () => {
  it('writes a BOM-prefixed header and one row per player, with CRLF line endings', () => {
    const csv = boardToCsv([row('Lamb')], null, new Set());
    expect(csv.startsWith('\uFEFFADP,Name,Pos,Team,Proj Pts,VORP,xADP,Delta,Drafted\r\n')).toBe(true);
  ...
  it('formats ADP/xADP/delta to one decimal, leaves nulls empty, and flags drafted players', () => {
    ...
    const csv = boardToCsv([r], null, new Set([r.player.id]));
  ...
  it('exports delta as a plain number without a plus sign', () => {
    ...
    expect(boardToCsv([r], null, new Set())).toContain('10.0,5.0,5.0,N');
  });
  ...
  it('quotes fields containing commas or quotes, doubling embedded quotes', () => {
    ...
    const csv = boardToCsv([weird], null, new Set());
...
describe('boardToCsv sorting', () => {
  it('sorts by column and direction, matching the visible table', () => {
    ...
    const asc = playerNames(boardToCsv(rows, { id: 'name', desc: false }, new Set()));
    expect(asc).toEqual(['Allen', 'Baker', 'Cousins']);
    const desc = playerNames(boardToCsv(rows, { id: 'name', desc: true }, new Set()));
  ...
  it('sorts numbers numerically', () => {
    ...
    const names = playerNames(boardToCsv(rows, { id: 'projectedPoints', desc: true }, new Set()));
  ...
  it('keeps nulls last for adp/xadp/delta in both directions (§6.4)', () => {
    ...
    for (const desc of [false, true]) {
      const names = playerNames(boardToCsv(rows, { id: 'adp', desc }, new Set()));
  ...
  it('leaves rows in original order for an unknown sort column', () => {
    ...
    const names = playerNames(boardToCsv(rows, { id: 'bogus', desc: false }, new Set()));
